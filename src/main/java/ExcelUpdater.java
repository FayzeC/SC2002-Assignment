...
public class ExcelUpdater {
    ...
    // Updates a specific cell in the Excel sheet based on header, new value, and
    // hospitalID
    public void updater(String filePath, String hospitalID, String header, String newValue) throws IOException {
        ...
        for (Row row : sheet) {
            ...
            if (idCell != null && idCell.getCellType() == CellType.STRING
                    && idCell.getStringCellValue().equals(hospitalID)) {
    ...
    public void updateInventory(String filePath, String medicineName, String header, String newValue)
        ...
        // Find the row with the matching medicineName
        boolean rowFound = false;
        for (Row row : sheet) {
            Cell nameCell = row.getCell(0); // Assuming the medicineName is in the first column
            if (nameCell != null && nameCell.getCellType() == CellType.STRING
                    && nameCell.getStringCellValue().equalsIgnoreCase(medicineName)) {
                Cell cellToUpdate = row.getCell(headerColumnIndex);
                if (cellToUpdate == null) {
                    cellToUpdate = row.createCell(headerColumnIndex);
                }
        ...
        // If no row with the specified medicineName was found, throw an exception
        if (!rowFound) {
            workbook.close();
            fis.close();
            throw new IllegalArgumentException("Medicine name \"" + medicineName + "\" not found in the Excel file.");
        }
